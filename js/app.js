...
while (sisterTriesRemaining > 0) {
  for (var i = 0; i < sisters.length; i++) {
    var sisterGuess = prompt('This final one is more challenging. Enter the name of one of my sisters.').toLowerCase();
    console.log('A correct sister guess can be averon, norseen, chana, or avery\nUser entered: ' + sisterGuess);

    if (sisterGuess === 'averon' || sisterGuess === 'norseen' || sisterGuess === 'chana' || sisterGuess === 'avery') {
      console.log(i);
      alert('Yep! She\'s one.');
      alert(sisters);
      break;
      ...
      if (sisterTriesRemaining === 0) {
        alert('Sorry, you\'re out of guesses. Here they are again:');
        alert(sisters);
        break;
      }
    }
